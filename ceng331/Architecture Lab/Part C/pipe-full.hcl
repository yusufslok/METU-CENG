# Yusuf Sami Lök 2521748. SUMMARY
#I will explain the changes I made line by line. 
#Firstly, in the writing section, when the IJTAB instruction is encountered, the valE value in memory is read and assigned to the valM value. 
#The reason for this assignment in the writing section is to have the address for the jump operation available, 
#obtained by reading the valM word in memory. 
#In the instruction valid section, as with any instruction, if an instruction equal to this value is encountered, it is for normal execution to continue.
#Since we are using the rb register, I added IJTAB to the need_regids section. 
#Additionally, since this instruction uses an 8-byte constant value, I added it to the need_valC section. 
#I added D_rb to the d_srcB section because D_rb will be used as d_srcB. In the aluA and aluB sections, 
#I added E_valc and E_valB respectively because constant value and source B are used in the addition operation. 
#As M_valE is used as the memory address, I added it to the relevant section. 
#The value read is actually the W_valM value, which we initially organized for the PC. I added mem_read for memory read. 
#To prevent hazards, in the decode section, if IJTAB is in the decode, execute, or memory sections, we should insert a bubble into the decode section. 
#This way, incorrectly read instructions cannot proceed to the decode section with a bubble and cannot pose any danger.
